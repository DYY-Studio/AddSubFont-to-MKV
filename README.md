...
此版本包括两个功能更新及少量问题修复
1. 添加了RmSubIn变量，现在可以选择是否保留输入文件中的内挂字幕了
2. 添加了RmAttach变量，现在可以选择是否保留输入文件中的附件了（如内嵌字体等）
3. 修复了在输入单个文件的情况下不嵌入字体的问题
4. 由于修复3所述问题，现在仅输入单个文件时也需要扫描该文件所在文件夹
5. 调整了轨道添加顺序，现在视频和音频轨道的序号不再比字幕轨道大了
6. 添加了titleVer变量，现在可以选择输出mkv文件中的标题是否需要添加ASFMKV的版本号了
### ExtL 1.05 （重要更新）
此版本包括一个重要问题修复和多个功能改进，建议立刻更新。
1. 修复了只输入一个文件而非文件夹时无法正常运行的问题
1. 完成了对UNC路径（或称 通用命名规则 / 局域网路径）的支持
1. 内置ReplaceBatch模块更新到V3F2_2
1. 内置ReplaceBatch模块已支持 新版CMD的命令扩展 提供的 Replace 方法，速度更快
### ExtL 1.04 （重要更新）
