...
if not defined debug (
	set debug=0
	rem INPUT_VALUE_LIST
	rem [范围][定义 P/定量 V/变量] 变量->对应变量 说明
	...
	rem [0/-0][V] %9->%9 DEBUG_MODE(Developing...)
...
if not defined notsub (
	echo [SUB][N] 若没有匹配的字幕则不封装视频
) else echo [SUB][A] 即使没有匹配到字幕也会封装视频（无字幕也嵌入字体）
...
if not defined ifolder (
	...
	if not exist "%ifolder:~1,-1%" (
		for %%a in ("%ifolder%") do set "ifolder=%%~a"
...
if "%nul_output%"=="0" (
	for /f "tokens=* delims=^|" %%a in ("%mkvinput%") do mkvmerge --ui-language "%ui_language%" -o NUL "%vpath%"%%~a || echo [DEBUG] 请等待5秒... && ping -n 5 -w 1000 127.0.0.1 >NUL
) else (
	mkvmerge --ui-language "%ui_language%" --priority "%priority%" -o "%opath%" --title "%filename%[ASFMKV%ver%_REMUX]" "%vpath%"%mkvinput% || echo [DEBUG] 请等待5秒... && ping -n 5 -w 1000 127.0.0.1 >NUL
)
:Encapsulation_OK
if exist "%opath%" (
