@rem !This file write by GB2312-80(zh-Hans) code page, do not open with other code pages. But you can even open this file with UTF-8 or UTF-16, these code pages can also show the texts!
@rem 关闭命令回显
@echo off
rem ------------------------------------------
...
for /f "tokens=1-2* delims= " %%a in ('mkvmerge -V') do (
	title [yyfll的批量混流程序]视频+字幕^&字体--^>MKV[%%a %%b] [GB2312] [VER. 1811GAMMA]
	echo [PL] 正在使用%%a %%b %%c
)
rem 如果已有输入就直接执行
echo.
...
rem 将符合搜索条件的文件的完整路径全部写入到filereader文件
...
rem 从filereader中读取所有文件路径并写入filelist
rem 如果定义了subdir变量等于0，则判断这些路径是否在目标目录而不是其子目录
rem (这是1811GAMMA为了减小磁盘读取压力而进行的一个没卵用的改进)
for /f "usebackq tokens=*" %%a in ("%~dp0filereader.log") do (
	if "%subdir%"=="0" (
		for /f "tokens=*" %%b in ("%ifolder%") do (
			if "%%~dpa"=="%%~b" (
				if exist "%%~a" (
