...
if not "%nul_output%"=="0" goto without_nul
call "%exein%" -o NUL --ui-language "%ui_language%" -v "%vpath%"%mkvinput%
if "%errorlevel%"=="2" (
	set /a error=error+1
	echo [DEBUG.mkvmerge] 正在重新运行并重定向mkvmerge输出...
	call "%exein%" -o NUL -v -r "%debug_file%" --ui-language "%ui_language%" "%vpath%"%mkvinput%
)
