...
if "%nul_output%"=="0" (
	for /f "tokens=* delims=^|" %%a in ("%mkvinput%") do (
		...
		if "%errorlevel%"=="2" (
			...
			mkvmerge -o NUL -v -r "%debug_file%" --ui-language "%ui_language%" "%vpath%"%%~a
		)
	)
) else (
	mkvmerge -o "%opath%" --ui-language "%ui_language%" --priority "%priority%" -v --title "[ASFMKV-%ver%] %filename%" "%vpath%"%mkvinput% 
	if not "%errorlevel%"=="0" (
		set /a error=error+1
		echo INPUT_FILE："%vpath%">"%debug_file%"
		echo [DEBUG.mkvmerge] 正在重新运行并重定向mkvmerge输出...
		mkvmerge -o NUL -r "%debug_file%" --ui-language "%ui_language%" --priority "%priority%" -v --title "[ASFMKV-%ver%] %filename%" "%vpath%"%mkvinput%
	)
)
...
echo   ?pri [1-3/input]    选择进程优先级
echo   ?uil [1-4/input]    选择Mkvmerge的语言
echo. 
echo 调用注意：
echo   开关必须用引号括起，比如
echo   "?output %APPDATA%\test" "?pri 3" "?dbm n:y"
echo.	
echo 调用方法①^(CMD^)：
...
echo   [2] normal^(标准^)
echo   [3] higher^(较高^)
echo   您也可以直接输入优先级，比如 "?pri highest"
echo   可选的优先级有 lowest^(最低^),lower^(较低^),normal^(标准^),higher^(较高^),highest^(最高/实时^)
echo.
echo MKVmerge输出语言：
echo   [1] en^(English^)
echo   [2] zh_CN^(简体中文^)
