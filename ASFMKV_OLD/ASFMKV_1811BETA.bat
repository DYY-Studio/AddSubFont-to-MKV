...
for /f "tokens=1-2* delims= " %%a in ('mkvmerge -V') do (
	title [yyfll的批量混流程序]视频+字幕^&字体--^>MKV[%%a %%b] [GB2312] [VER. 1811BETA]
	echo [PL] 正在使用%%a %%b %%c
)
rem 如果已有输入就直接执行
echo.
...
for /f "tokens=2 delims=^&" %%a in ("%subname%A") do (
	if not "%%a"=="" (
		for %%b in ("%subname%") do (
			call :Module_DeleteAND "%%~nb" && call :Rename_Sub "%%~xb"
		)
	)
)
...
for /f "tokens=2 delims=^&" %%a in ("%fontname%A") do if not "%%b"=="" for %%a in ("%fontname%") do call :Module_DeleteAND "%%~na" && call :Rename_Font "%%~xa"
