...
echo [INF] AddSubFont(to)MKV V%ver% ^| Copyright(c) 2018-2019 By yyfll
for /f "tokens=1-2* delims= " %%a in ('mkvmerge -V') do (
	title [yyfll的批量混流程序]视频+字幕^&字体--^>MKV[%%a %%b] [GB2312] [VER. %ver%]
...
if not defined notsub (
	echo [SUB][N] 若没有视频没有附带的字幕则不会再封装视频
) else echo [SUB][A] 即使视频没有字幕也会再封装（无字幕也嵌入字体）
if not defined notfont (
	echo [FNT][N] 不封装字体（仅嵌入字幕）
) else echo [FNT][A] 封装字体（同时嵌入匹配字幕与字体）
if defined outpath echo [OUT][A] 封装后的媒体文件将会输出到"%outpath%"
if "%subdir%"=="0" echo [DIR][A] SubDir已被定义为不搜索目标目录下的子目录
